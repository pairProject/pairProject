...
public class CashierOperationModule
{
    ...
    private void doCheckout()
    {
        ...
        if(checkoutControllerRemote.getTotalLineItem() > 0)
        {
            ...
            if(confirmCheckout.equals("Y"))
            {
                try
                {
                    SaleTransactionEntity newSaleTransactionEntity = checkoutControllerRemote.doCheckout(currentStaffEntity);
    ...
    //                            emailControllerRemote.emailCheckoutNotificationSync(newSaleTransactionEntity, "Tan Wee Kek<tanwk@comp.nus.edu.sg>", toEmailAddress);

    //                             02 - Asynchronous Session Bean Invocation
                            asyncResult = emailControllerRemote.emailCheckoutNotificationAsync(newSaleTransactionEntity, "Tan Wee Kek<tanwk@comp.nus.edu.sg>", toEmailAddress);
    ...
    //                            sendJMSMessageToQueueCheckoutNotification(newSaleTransactionEntity.getSaleTransactionId(), "Tan Wee Kek<tanwk@comp.nus.edu.sg>", toEmailAddress);
